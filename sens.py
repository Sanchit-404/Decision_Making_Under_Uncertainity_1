...
def inject_noise(data, noise_level=0.1):
    ...
    for col in noisy_data.columns:
        if col != 'Admit' and col != 'Research':
            # Calculate the range of the column
            column_range = noisy_data[col].max() - noisy_data[col].min()
            # Determine the noise standard deviation based on the column range
            noise_std = column_range * noise_level
            # Generate noise and add it to the column
            noise = np.random.normal(0, noise_std, noisy_data[col].shape)
            noisy_data[col] += noise
            # Ensure no negative values
            noisy_data[col] = noisy_data[col].clip(0)
    return noisy_data
...
for gre_bins in gre_bins_list:
    for toefl_bins in toefl_bins_list:
        for gpa_bins in gpa_bins_list:
            ...
            train_data, temp_data = train_test_split(data_disc.drop('Chance of Admit ', axis=1), test_size=0.2, stratify=data_disc['Admit'], random_state=42)
            test_data, noisy_data = train_test_split(temp_data, test_size=0.5, stratify=temp_data['Admit'], random_state=42)
            ...
            noisy_data = inject_noise(noisy_data, noise_level=0.1)
            X_noisy_test = noisy_data.drop('Admit', axis=1)
            y_true_noisy = noisy_data['Admit']
            y_pred_noisy = []

            for _, row in X_noisy_test.iterrows():
                evidence = {col: int(row[col]) for col in X_noisy_test.columns}
                try:
                    pred = inference.map_query(['Admit'], evidence=evidence)
                    y_pred_noisy.append(pred['Admit'])
                except IndexError:
                    y_pred_noisy.append(y_true_noisy.mode()[0])

            accuracy_noisy = accuracy_score(y_true_noisy, y_pred_noisy)
            f1_noisy = f1_score(y_true_noisy, y_pred_noisy)
            ...
            results.append({
                ...
                'Accuracy (Noisy)': accuracy_noisy,
                'F1 Score (Noisy)': f1_noisy,
                ...
                'Classification Report (Noisy)': json.dumps(classification_report(y_true_noisy, y_pred_noisy, output_dict=True))
            })

            print(f"Accuracy (Clean): {accuracy_clean:.4f}, F1 Score (Clean): {f1_clean:.4f}")
            print(f"Accuracy (Noisy): {accuracy_noisy:.4f}, F1 Score (Noisy): {f1_noisy:.4f}")
            print("Classification Report (Clean):")
            print(classification_report(y_true, y_pred))
            print("Classification Report (Noisy):")
            print(classification_report(y_true_noisy, y_pred_noisy))
...
print("\nSummary of Results:")
print(results_df[['GRE bins', 'TOEFL bins', 'GPA bins', 'Accuracy (Clean)', 'F1 Score (Clean)', 'Accuracy (Noisy)', 'F1 Score (Noisy)']])
