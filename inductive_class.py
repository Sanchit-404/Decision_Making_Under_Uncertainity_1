...
def discretize_and_augment_data(data, gre_bins, toefl_bins, gpa_bins):
    ...
    return data_disc[['GRE Score_cat', 'TOEFL Score_cat', 'CGPA_cat', 'Research', 'SOP', 'LOR ', 'University Rating', 'Non_Academic_Evaluation', 'Final_Academics', 'Admit']]
    ...
    for toefl_bins in toefl_bins_list:
        for gpa_bins in gpa_bins_list:
            ...
            for _, row in X_test.iterrows():
                evidence = {col: int(row[col]) for col in X_test.columns if col in model.nodes()}
                try:
            ...
            results.append({
                ...
                'F1 Score': f1,
                'Model': 'Inductive Bias BN with SMOTE',
...
# Save results to CSV
results_df.to_csv('inductive_bias_bn_model_results_with_smote.csv', index=False)
print("\nResults saved to 'inductive_bias_bn_model_results_with_smote.csv'")
